...
export default function TreasuryPanel({ initialPayments, registrationFeeCount, registrationFeeTotal }: { initialPayments: Payment[]; registrationFeeCount: number; registrationFeeTotal: number }) {
  const [payments, setPayments] = useState(initialPayments)
  const [form, setForm] = useState({ amount: '', type: 'income', status: 'recorded' })
  ...
  useEffect(() => {
    ...
    const refresh = async () => {
      try {
        ...
        if (active && Array.isArray(data)) setPayments(data)
      } catch {
        // Keep the last known ledger visible if a refresh fails.
      }
    }
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setForm({ amount: '', type: 'income', status: 'recorded' })
      setEditingId(null)
  ...
  const editPayment = (payment: Payment) => {
    ...
    setForm({ amount: String(payment.amount), type: payment.type, status: payment.status })
  }
  ...
  return (
    <>
      <section style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 12, marginTop: 24 }}>
        ...
        {['Income', `KSh ${income.toLocaleString()}`, 'Expenses', `KSh ${expenses.toLocaleString()}`, 'Balance', `KSh ${balance.toLocaleString()}`, 'Fees paid this year', String(registrationFeeCount), 'Fee income this year', `KSh ${registrationFeeTotal.toLocaleString()}`].reduce<Array<[string, string]>>((cards, value, index, values) => index % 2 === 0 ? [...cards, [value, values[index + 1]]] : cards, []).map(([label, value]) => <div key={label} style={{ background: '#ffffff', border: `1px solid ${C.border}`, borderRadius: 10, padding: 16 }}><div style={{ color: C.muted, fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.1em' }}>{label}</div><div style={{ color: C.text, fontSize: 24, marginTop: 6 }}>{value}</div></div>)}
      </section>
      ...
      <section style={{ background: '#ffffff', border: `1px solid ${C.border}`, borderRadius: 12, padding: 20, marginTop: 20 }}>
        ...
        <form onSubmit={submit} style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 10 }}>
          ...
          {editingId ? <button type="button" onClick={() => { setEditingId(null); setForm({ amount: '', type: 'income', status: 'recorded' }) }} style={{ border: `1px solid ${C.border}`, borderRadius: 8, background: '#fff', color: C.text }}>Cancel</button> : null}
        </form>
