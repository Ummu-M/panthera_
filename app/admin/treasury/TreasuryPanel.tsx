...
export default function TreasuryPanel({ initialPayments }: { initialPayments: Payment[] }) {
  ...
  const [form, setForm] = useState({ amount: '', type: 'income', purpose: '' })
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setForm({ amount: '', type: 'income', purpose: '' })
      setEditingId(null)
  ...
  const editPayment = (payment: Payment) => {
    ...
    setForm({ amount: String(payment.amount), type: payment.type, purpose: payment.purpose || '' })
  }
    ...
    <>
      ...
      <section style={{ background: '#ffffff', border: `1px solid ${C.border}`, borderRadius: 12, padding: 20, marginTop: 20 }}>
        ...
        <form onSubmit={submit} style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 10 }}>
          ...
          <input required placeholder="Purpose" value={form.purpose} onChange={(event) => setForm({ ...form, purpose: event.target.value })} style={{ padding: 11, border: `1px solid ${C.border}`, borderRadius: 8 }} />
          ...
          {editingId ? <button type="button" onClick={() => { setEditingId(null); setForm({ amount: '', type: 'income', purpose: '' }) }} style={{ border: `1px solid ${C.border}`, borderRadius: 8, background: '#fff', color: C.text }}>Cancel</button> : null}
        </form>
      ...
      <section style={{ background: '#ffffff', border: `1px solid ${C.border}`, borderRadius: 12, overflowX: 'auto', marginTop: 20 }}>
        <table style={{ width: '100%', minWidth: 650, borderCollapse: 'collapse', color: C.text }}>
          <thead><tr style={{ background: 'rgba(46,155,236,0.06)' }}><th style={{ textAlign: 'left', padding: 14 }}>Amount</th><th style={{ textAlign: 'left', padding: 14 }}>Type</th><th style={{ textAlign: 'left', padding: 14 }}>Purpose</th><th style={{ textAlign: 'left', padding: 14 }}>Recorded by</th><th style={{ textAlign: 'left', padding: 14 }}>Date</th><th style={{ textAlign: 'left', padding: 14 }}>Action</th></tr></thead>
          <tbody>{payments.map((payment) => <tr key={payment.id}><td style={{ padding: 14 }}>KSh {Number(payment.amount).toLocaleString()}</td><td style={{ padding: 14 }}>{payment.type}</td><td style={{ padding: 14 }}>{payment.purpose || '—'}</td><td style={{ padding: 14 }}>{payment.userId}</td><td style={{ padding: 14 }}>{new Date(payment.createdAt).toLocaleDateString()}</td><td style={{ padding: 14 }}><button type="button" onClick={() => editPayment(payment)} style={{ border: 'none', background: 'transparent', color: C.gold600, cursor: 'pointer' }}>Edit</button><button type="button" onClick={() => removePayment(payment.id)} style={{ border: 'none', background: 'transparent', color: '#9b2c2c', cursor: 'pointer', marginLeft: 10 }}>Remove</button></td></tr>)}</tbody>
        </table>
      </section>
    </>
  )
